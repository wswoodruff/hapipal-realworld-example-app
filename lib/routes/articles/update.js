...
module.exports = Helpers.withDefaults({
    ...
    options: {
        ...
        handler: async (request, h) => {
            ...
            const loadedArticle = await articleService.fetchArticlesByUser(currentUserId, article);

            return {
                article: ArticleDTO.updateResponse.from(loadedArticle)
            };

            // const [betterArticle] = await articleService.fetchArticlesByUser(currentUserId, article);

            // return {
            //     article: await ArticleDTO.detailed.from(betterArticle)
            // };
        }
    }
});
