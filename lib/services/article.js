...
module.exports = class ArticleService extends Schmervice.Service {
    ...
    async fetchArticlesByUser(userId, articles, txn) {
        ...
        const toArticle = ({ tags, favorited, favoritesCount, author, ...article }) => ({
            ...article,
    ...
    async update(id, dto, txn) {
        ...
        const { Article, Tag } = this.server.models();
        // Validates, warms your <3
        const { tagList, ...articleInfo } = ArticleDTO.update.from(dto);
